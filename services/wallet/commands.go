...
func (c *ethHistoricalCommand) Run(ctx context.Context) (err error) {
	...
	log.Info("balance cache", "total", totalRequests, "cached", cacheHits)
	...
	log.Info("eth historical downloader finished successfully", "address", c.address, "from", c.from, "to", c.to, "total transfers", len(transfers), "total blocks", len(blocks), "blocks", blocks, "time", time.Since(start))
	totalRequests, cacheHits = c.balanceCache.getStats(c.address)
	log.Info("balance cache", "total", totalRequests, "cached", cacheHits)
