...
func (c *ethHistoricalCommand) Run(ctx context.Context) (err error) {
	...
	from, blocks, err := findBlocksWithEthTransfers(ctx, c.client, c.balanceCache, c.eth, c.address, c.from, c.to)

	if err != nil {
		return err
	}

	log.Info("eth historical downloader finished successfully", "address", c.address, "from", from, "to", c.to, "total blocks", len(blocks), "blocks", blocks, "time", time.Since(start))
	totalRequests, cacheHits = c.balanceCache.getStats(c.address)
	log.Info("balance cache after checking range", "total", totalRequests, "cached", totalRequests-cacheHits)
	err = c.db.ProcessBlocks(c.address, from, c.to, blocks, ethTransfer)
	if err != nil {
...
func (c *newBlocksTransfersCommand) Run(parent context.Context) (err error) {
	...
	err = c.db.ProcessTranfers(all, removed, erc20Sync|ethSync)
	if err != nil {
