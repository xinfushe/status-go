...
func (db Database) ProcessTranfers(transfers []Transfer, removed []*DBHeader, option SyncOption) (err error) {
	var (
		tx *sql.Tx
	)
	tx, err = db.db.Begin()
...
func (db *Database) GetTransfersByAddressAndPage(address common.Address, fromBlock *big.Int, pageSize int64) (rst []Transfer, err error) {
...
func deleteHeaders(creator statementCreator, headers []*DBHeader) error {
	delete, err := creator.Prepare("DELETE FROM blocks_with_transactions WHERE blk_hash = ?")
