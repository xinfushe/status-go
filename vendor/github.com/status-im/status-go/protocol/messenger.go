			...
			for _, msg := range statusMessages {
				...
				if msg.ParsedMessage != nil {
					if textMessage, ok := msg.ParsedMessage.(*protobuf.ChatMessage); ok {
						...
						receivedMessage.From = contact.ID
						receivedMessage.SigPubKey = publicKey
						receivedMessage.Alias = contact.Alias
						receivedMessage.Identicon = contact.Identicon
						receivedMessage.WhisperTimestamp = uint64(msg.TransportMessage.Timestamp) * 1000
