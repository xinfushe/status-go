...
func (m *Messenger) handleRetrievedMessages(chatWithMessages map[transport.Filter][]*types.Message) (*MessengerResponse, error) {
	...
	for _, c := range contacts {
		logger.Info("Checking blocked contact", zap.Any("system-tags", c.SystemTags), zap.Bool("blocked", c.IsBlocked()), zap.String("contact-id", c.ID))
		if c.IsBlocked() {
	...
	for chat, messages := range chatWithMessages {
		for _, shhMessage := range messages {
			...
			for _, msg := range statusMessages {
				...
				logger.Info("Checking against blocked", zap.String("sender-id", senderID))
